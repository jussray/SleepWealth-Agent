...
RUNTIME_BUNDLE_IDENTITY_FILE = "backend/runtime_bundle_identity.py"
...
RUNTIME_FILES = (
    ...
    "backend/runtime_bundle_identity.py",
...
def _write_bundle_identity(destination: Path, source_sha: str) -> None:
    destination.write_text(
        "from __future__ import annotations\n\n"
        "# Generated by scripts/package_vercel_runtime.py.\n"
        f'SOURCE_SHA: str | None = "{source_sha}"\n'
...
def package_runtime(root: Path, output: Path, source_sha: str | None = None) -> dict:
    ...
    for relative in RUNTIME_FILES:
        ...
        if relative == RUNTIME_BUNDLE_IDENTITY_FILE:
            _write_bundle_identity(destination, source_sha)
        else:
