...
_ALLOWED_SOURCE_CLASSIFICATIONS = frozenset(
    {
        "authorized-api",
        "public-chain-read-only",
        "user-supplied",
    }
)
...
class PumpSnapshotProvider:
    ...
    async def get_market_data(self, symbol: str) -> dict[str, Any]:
        ...
        if requested != self.snapshot["symbol"]:
            raise ValueError(
                f"snapshot is for {self.snapshot['symbol']}, not requested symbol {requested}"
            )
        return {
...
class PumpShadowBridge:
    ...
    def mirror_to_practice(
        ...
        *,
        ...
        mirror_fingerprint = sha256(encoded).hexdigest()
