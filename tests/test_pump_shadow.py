...
async def test_website_scrape_classification_is_rejected():
    with pytest.raises(PermissionError, match="scraping is not supported"):
        await PumpShadowBridge.observe(
            snapshot(source_classification="pump-site-scrape")
        )


@pytest.mark.asyncio
async def test_snapshot_identity_cannot_be_substituted():
    provider_snapshot = snapshot(symbol="MOM8")
    live = await PumpShadowBridge.observe(provider_snapshot)
    assert live.symbol == "MOM8"

    sandbox = CryptoSandboxBroker(initial_cash=10.0)
    await sandbox.connect()
    receipt = PumpShadowBridge.mirror_to_practice(
        live,
        mint=provider_snapshot["mint"],
        sandbox=sandbox,
    )

    assert receipt.symbol == "MOM8"
    assert receipt.mint == provider_snapshot["mint"]
